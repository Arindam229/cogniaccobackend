...
const TestMailParticipant = require('../models/TestMailParticipant'); // ← Changed
...
const processJob = async (jobId, participants) => {
  ...
  for (let i = 0; i < participants.length; i++) {
    ...
    const uniqueID = `QUEUE-${Date.now()}-${Math.floor(Math.random() * 10000)}`;
    
    try {
      // Send email...
      await sendEmail(email, name,phoneNumber, collegeName, uniqueID);
      
      // Save to TestMailParticipant collection (NOT Participant)
      await TestMailParticipant.create({ 
        uniqueID,
        name, 
        email,
      ...
      console.log(`✅ Queue: Sent ${i + 1}/${participants.length}: ${email}`);
